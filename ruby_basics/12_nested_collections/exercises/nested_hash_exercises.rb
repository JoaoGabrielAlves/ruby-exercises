...
def add_information_about_language(languages, language_name, info_key, info_value)
  ...
  languages[language_name].store(info_key, info_value)
...
def add_language(languages, language_name, language_info_value)
  ...
  languages.store(language_name, language_info_value)
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  if languages.key?(language_name)
    facts_array = languages[language_name][:facts]

    puts facts_array

    if fact_index >= 0 && fact_index < facts_array.length
      return facts_array[fact_index]
    end
